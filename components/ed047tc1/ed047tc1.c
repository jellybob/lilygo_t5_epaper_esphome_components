...
#include <soc/gpio_struct.h>  // GPIO.out_w1ts / GPIO.out_w1tc
...
void epd_poweroff()
{
    // Set config registers to disable power lines and
    // push to the display controller.
    config_reg.pos_power_enable = false;
    push_cfg(&config_reg);
    busy_delay(10 * 240);
    config_reg.neg_power_enable = false;
    ...
    // Zero all config registers. This prevents floating
    // values from causing display corruption once power
    // is removed.
